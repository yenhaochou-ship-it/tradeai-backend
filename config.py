...
DAYTRADE_TAX_RATE = 0.0015  # 個股當沖證交稅 0.15%（優惠至2027年底，賣出收一次）
ETF_TAX_RATE = 0.001  # ETF證交稅0.1%——這是ETF本來就有的優惠稅率(跟是不是當沖無關)，
                       # 「現股當沖證交稅減半」這個優惠規則本身就不適用在ETF上，ETF當沖一樣是0.1%，
                       # 沒有再打對折。跟個股當沖0.15%是兩條獨立規則，只是剛好ETF原本就比較低。

def is_etf(symbol:str) -> bool:
    """台股ETF代號慣例都是「00」開頭(0050、0056、006208、00878、00631L、009816...)，
    一般個股代號是4位數字(2330、2454...)不會有這個開頭——用這個規則判斷夠用，不需要額外查表。"""
    return symbol.replace(".TW","").replace(".TWO","").startswith("00")

def _tax_rate_for(symbol:str=None) -> float:
    """修正：原本不分股票/ETF一律用個股當沖稅率0.15%計算成本——但00631L/009816加入交易池後，
    這樣會幫這兩檔多算0.05個百分點的稅，讓模擬損益系統性偏保守，也會讓min_profitable_move_pct
    的門檻在ETF上設得比實際需要更嚴，可能讓本來划算的ETF交易被誤判成「漲跌幅不夠」而被濾掉。
    symbol不給(維持舊呼叫方式)時退回個股稅率，向後相容。"""
    return ETF_TAX_RATE if symbol and is_etf(symbol) else DAYTRADE_TAX_RATE

def calc_round_trip_cost(entry_price:float, exit_price:float, qty:int, fee_discount:float=FEE_DISCOUNT, symbol:str=None) -> dict:
    ...
    symbol給的話會自動依股票/ETF套用對的證交稅率(見_tax_rate_for)。"""
    ...
    tax      = sell_amt*_tax_rate_for(symbol)
...
def min_profitable_move_pct(fee_discount:float=FEE_DISCOUNT, symbol:str=None) -> float:
    """當沖至少要漲跌多少%才能扣成本後還有獲利。同樣接受fee_discount覆寫，理由跟上面一致。
    symbol給的話ETF會算出比較低的門檻(約0.27% vs 個股0.32%，因為ETF證交稅比較低)。"""
    return (FEE_RATE*fee_discount*2 + _tax_rate_for(symbol))*100
