...
def _lgbm_meta_path() -> str:
    """中繼資料(AUC/準確率/特徵重要性等)的檔案路徑，跟模型檔同一個資料夾、同名只是換副檔名——
    train_lgbm_model.py訓練完會存這個檔案，main.py這邊只負責讀，不負責產生。"""
    base = LGBM_MODEL_PATH.rsplit(".", 1)[0] if "." in LGBM_MODEL_PATH else LGBM_MODEL_PATH
    return base + "_meta.json"

def get_lgbm_model_meta() -> Optional[Dict]:
    """讀取訓練時存的中繼資料(AUC/準確率/樣本數/特徵重要性等)，給前端顯示「目前這個模型實際多準」，
    不用再叫使用者自己回頭找訓練時的終端機輸出。這份檔案是這個功能上線後才開始產生的——
    在這之前訓練出來的模型沒有這個檔案，讀不到就回傳None，前端要顯示「沒有記錄」而不是當成錯誤。"""
    path = _lgbm_meta_path()
    ...
    try:
        ...
        logger.warning(f"讀取LightGBM模型中繼資料失敗: {e}")
        return None
