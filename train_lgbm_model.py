...
"""
import argparse, os, sys, bisect, json
...
CACHE_DIR = "hist_cache"  # 本地磁碟快取歷史K棒，不進git(.gitignore已排除)——
                          # 1分K棒一旦交易日結束就不會再變動，快取起來下次只抓「新增的部分」，
                          # 不用每次重訓都把整段歷史(90天*48檔)從永豐重新抓一遍。
...
SCAN_UNIVERSE = [
    ...
    # 跟config.py的SCAN_UNIVERSE保持一致(這份是手動複製的，main.py改了這裡要記得跟著改)：
    # 00631L元大台灣50正2(2倍槓桿)、009816凱基台灣TOP50(2026年初掛牌，無槓桿)。
    "00631L","009816",
]
...
def _load_cache(symbol):
    """讀本地快取，檔案不存在/壞掉都當成「沒有快取」處理，不會讓整個訓練因此中斷——
    最差情況只是退回完整重抓那一檔，跟原本沒有快取功能時的行為一樣。"""
    ...
    try:
        ...
        print(f"  ⚠️ {symbol} 快取檔案讀取失敗(可能損毀)，這次當作沒有快取重新抓: {e}")
        return []

def _save_cache(symbol, bars):
    os.makedirs(CACHE_DIR, exist_ok=True)
    try:
        with open(_cache_path(symbol), "w", encoding="utf-8") as f:
            json.dump(bars, f)
    except Exception as e:
        print(f"  ⚠️ {symbol} 快取存檔失敗(不影響這次訓練的結果，只是下次又要重新抓這檔): {e}")

def _fetch_kbars_range(api, contract, symbol, start_d, end_d):
    """實際呼叫永豐kbars()抓一段日期範圍，內部依30天上限切成多段(每段最多28天)分批抓，
    單一段失敗不影響其他段。回傳(這段範圍內所有K棒-未去重, 失敗段數, 總段數)。"""
    ...
    return all_bars, failed_chunks, total_chunks

def fetch_1min_bars(api, symbol, days, use_cache=True):
    ...
    原本直接拿days(預設90)整段去問，全部都會失敗。改成切成多段(_fetch_kbars_range)分批抓。
    延續法：原本每次重訓都把整段days天數從永豐重新抓一遍，現在改成先看本地快取(hist_cache/)
    有沒有上次抓過的資料——1分K棒一旦交易日結束就不會再變動，已經抓過的舊資料可以一路沿用，
    只需要抓「快取最後一筆之後到現在」這一小段新增的，明顯減少要打給永豐的請求量跟等待時間。
    如果快取的歷史深度不夠這次要求的days(例如上次只抓30天，這次要90天)，深度不足的部分
    沒辦法用「抓新的」補回來(缺的是更早以前的，不是新的)，這種情況退回完整重抓整段，
    跟原本沒有快取功能時一樣，不會讓你看起來「有資料」但其實深度不夠。"""
    ...
    cached = _load_cache(symbol) if use_cache else []
    if cached:
        earliest_cached = datetime.fromtimestamp(min(b["ts"] for b in cached), tz=TW_TZ)
        latest_cached = datetime.fromtimestamp(max(b["ts"] for b in cached), tz=TW_TZ)
        if earliest_cached <= start_d + timedelta(days=1):  # 快取深度夠用(留1天容錯，避開邊界)
            # 從「快取最後一筆的前一天」開始重抓(留1天重疊，當天可能上次抓的時候盤還沒收，
            # 重疊抓到的舊資料靠下面timestamp去重會自動蓋過去，不會變成兩倍)，到現在為止。
            fetch_from = max(latest_cached - timedelta(days=1), start_d)
            new_bars, failed_chunks, total_chunks = _fetch_kbars_range(api, contract, symbol, fetch_from, end_d)
            all_bars = cached + new_bars
            cache_note = f"(延續法：沿用快取{len(cached)}根+新抓{len(new_bars)}根)"
        else:
            print(f"  ℹ️ {symbol} 快取深度不足(快取最早只到{earliest_cached.strftime('%Y-%m-%d')}，"
                  f"這次要求回溯到{start_d.strftime('%Y-%m-%d')})，這檔改成完整重抓")
            all_bars, failed_chunks, total_chunks = _fetch_kbars_range(api, contract, symbol, start_d, end_d)
            cache_note = "(快取深度不足，完整重抓)"
    else:
        all_bars, failed_chunks, total_chunks = _fetch_kbars_range(api, contract, symbol, start_d, end_d)
        cache_note = "(無快取，完整抓取)"
    ...
    if use_cache:
        _save_cache(symbol, bars)  # 存完整版(含比這次days更早的歷史)，下次即使--days調大也能少抓一點
    start_ts = start_d.timestamp()
    bars = [b for b in bars if b["ts"] >= start_ts]  # 回傳時才裁切成這次實際要求的範圍
    status = "" if failed_chunks==0 else f"（{failed_chunks}/{total_chunks}段失敗，資料可能不完整）"
    print(f"  ✓ {symbol}: {len(bars)}根1分K棒（{start_d.strftime('%Y-%m-%d')}~{end_d.strftime('%Y-%m-%d')}）{cache_note}{status}")
    return bars
...
def _build_samples_for_symbol(sym, bars, market_entries, market_ts_list, stride):
    """單一股票的特徵+標籤建構，抽成獨立的模組層級函式——這是build_dataset()原本內層for迴圈的內容，
    抽出來才能被ProcessPoolExecutor序列化送進worker處理程序(multiprocessing對函式的要求比
    ThreadPoolExecutor嚴格，一定要是模組頂層、可以被import到的函式，不能是內層定義/closure)。
    回傳(sym, X_sym, y_sym, meta_sym)，呼叫端要負責照symbols原本的順序組回去(見下方說明)。"""
    if len(bars) < 60:
        return sym, [], [], []
    ...
    for i in range(30, len(bars) - LABEL_MAX_HOLD_MIN - 1, stride):
        ...
        X_sym.append(feats); y_sym.append(label)
        meta_sym.append({"sym": sym, "idx": i})
    return sym, X_sym, y_sym, meta_sym

def build_dataset(api, symbols, days, stride=5, max_workers=5, use_cache=True, feature_workers=0):
    """對每檔股票，每隔stride根棒子取一個樣本點(不用每一根都取，減少高度重疊的樣本、加快訓練)，
    算特徵+標籤，組成完整訓練資料。
    修正①：原本fetch_1min_bars()對每檔股票是依序呼叫，48檔*每檔約4段(90天/28天上限)
    =接近200次依序網路請求，每次都要等永豐回應才能打下一個，實測要十幾分鐘到半小時。
    這些股票的歷史資料互不相關，改成用ThreadPoolExecutor同時抓max_workers檔——這段是網路
    I/O等待(不是CPU運算)，Python的GIL在等網路回應時會釋放，多執行緒確實能讓總時間明顯縮短，
    不是假平行。⚠️ 沒辦法確認永豐Shioaji SDK對同一個api物件被多執行緒同時呼叫kbars()是否
    完全保證安全(官方文件沒明確講)，保守起見max_workers預設只開5，不是越高越好——如果改大後
    開始看到一堆「抓取失敗」的警告變多，那是撞到永豐自己的併發限制，調低這個數字就好，
    不是程式壞了。每檔股票各自的fetch_1min_bars()內部仍然會把失敗的段落單獨印出來，
    不會因為平行化就看不出哪一段抓壞了。
    修正②：抓資料變快之後，「建立特徵+標籤」這段(每檔股票上萬根K棒、每隔stride根算一次特徵)
    變成新的瓶頸——這段是純CPU運算，不是網路等待，ThreadPoolExecutor在這裡完全沒用
    (Python的GIL會讓CPU密集的多執行緒形同虛設，多個執行緒還是輪流用同一顆核心)，
    必須用ProcessPoolExecutor(多進程，每個進程有自己的GIL，才能真正同時用多顆CPU核心)。
    ⚠️ 用多進程平行處理後，每檔股票完成的順序不再保證跟symbols清單原本的順序一樣
    (哪個進程先算完就先回來)，但下面組裝X/y/meta時刻意照symbols原本的順序組回去，
    確保train/valid的80/20時間序切分跟沒有平行化之前的結果完全一樣，不會因為平行化
    改變了訓練資料的順序、進而改變了切分結果——只是處理過程變快，結果應該要一致。"""
    # 先抓0050的歷史K棒，用來算每個樣本點當下的「大盤同步性」特徵——只抓一次，所有股票共用，
    # 不是每檔股票各自抓一份(0050的走勢對所有股票來說是同一份外部資料，不需要重複抓)。
    print("抓取大盤代理指標(0050)歷史資料...")
    market_bars = fetch_1min_bars(api, MARKET_INDEX_SYMBOL, days, use_cache=use_cache)
    ...
    print(f"平行抓取{len(symbols)}檔歷史資料(同時{max_workers}檔，原本是一檔一檔依序抓)...")
    bars_by_sym = {}
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_sym = {executor.submit(fetch_1min_bars, api, sym, days, use_cache): sym for sym in symbols}
        done_count = 0
        for future in as_completed(future_to_sym):
            sym = future_to_sym[future]
            done_count += 1
            try:
                bars_by_sym[sym] = future.result()
            except Exception as e:
                # 平行抓取時某一檔整個拋出例外(不是fetch_1min_bars內部已經接住的單段失敗)，
                # 不能讓這個錯誤波及到其他還在跑的執行緒，記錄成這檔沒資料，其他檔照樣跑完。
                print(f"  ⚠️ {sym} 抓取時發生未預期錯誤，這檔本次訓練不會有資料: {e}")
                bars_by_sym[sym] = []
            print(f"  [{done_count}/{len(symbols)}] 完成抓取")

    n_proc = feature_workers or min(os.cpu_count() or 4, len(symbols))
    print(f"建立訓練樣本(平行處理，同時{n_proc}個處理程序)...")
    results_by_sym = {}
    with ProcessPoolExecutor(max_workers=n_proc) as executor:
        future_to_sym = {
            executor.submit(_build_samples_for_symbol, sym, bars_by_sym.get(sym, []),
                             market_entries, market_ts_list, stride): sym
            for sym in symbols
        }
        done_count = 0
        for future in as_completed(future_to_sym):
            ...
            done_count += 1
            try:
                _sym, X_sym, y_sym, meta_sym = future.result()
                results_by_sym[sym] = (X_sym, y_sym, meta_sym)
                print(f"  [{done_count}/{len(symbols)}] {sym}: {len(X_sym)} 個訓練樣本")
            except Exception as e:
                print(f"  ⚠️ {sym} 建立特徵時發生未預期錯誤，這檔本次訓練不會有樣本: {e}")
                results_by_sym[sym] = ([], [], [])

    # 照symbols原本的順序組裝回去(不是用上面完成的順序)，理由見上面docstring「修正②」的說明。
    X, y, meta = [], [], []
    for sym in symbols:
        X_sym, y_sym, meta_sym = results_by_sym.get(sym, ([], [], []))
        X.extend(X_sym); y.extend(y_sym); meta.extend(meta_sym)
    return X, y, meta
...
def main():
    ...
    ap.add_argument("--workers", type=int, default=5, help="平行抓取歷史資料的同時執行緒數(預設5)；"
                     "如果常看到「抓取失敗」警告變多，可能是撞到永豐API的併發限制，調低這個數字")
    ap.add_argument("--no-cache", action="store_true", help="不使用hist_cache/本地快取，強制完整重新抓取所有歷史資料"
                     "(快取檔案壞掉/懷疑資料有問題時用這個排除快取造成的因素)")
    ap.add_argument("--feature-workers", type=int, default=0, help="建立特徵/標籤時平行處理的處理程序數"
                     "(預設0=自動，取CPU核心數跟股票數的較小值)；這段是純CPU運算，跟--workers"
                     "(抓資料用的執行緒數)是不同的瓶頸，調這個不會影響抓資料速度，互不相關")
    ...
    X, y, meta = build_dataset(api, symbols, args.days, args.stride, max_workers=args.workers,
                                use_cache=not args.no_cache, feature_workers=args.feature_workers)
    ...
    # 存一份中繼資料(AUC/準確率/特徵重要性/新舊比較)到旁邊的_meta.json，讓main.py可以讀出來
    # 轉發給前端顯示——不用每次都要回頭看這次訓練在終端機印出來的那一大段文字才知道模型多準。
    # 跟模型檔案放同一個資料夾、同名只是副檔名換成_meta.json，跟著args.output一起部署到後端即可。
    meta_path = (args.output.rsplit(".", 1)[0] if "." in args.output else args.output) + "_meta.json"
    meta = {
        "trained_at": tw_now().isoformat(),
        "auc": round(float(auc), 4),
        "accuracy": round(float(acc), 4),
        "n_samples": len(X),
        "positive_ratio": round(float(sum(y) / len(y)), 4) if len(y) else None,
        "days": args.days,
        "n_symbols": len(symbols),
        "stride": args.stride,
        "old_auc": round(float(old_auc), 4) if old_auc is not None else None,
        "auc_diff": round(float(auc - old_auc), 4) if old_auc is not None else None,
        "real_data_used": bool(args.feature_log_csv),
        "feature_importance": [[name, int(imp)] for name, imp in importance],
    }
    try:
        with open(meta_path, "w", encoding="utf-8") as f:
            json.dump(meta, f, ensure_ascii=False, indent=2)
        print(f"✅ 訓練中繼資料已存到 {meta_path}（前端「系統」分頁的模型狀態會讀這個檔案顯示AUC等資訊）")
    except Exception as e:
        print(f"⚠️ 中繼資料存檔失敗(不影響模型本身，只是前端會看不到這次的AUC等資訊): {e}")
