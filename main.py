...
模組拆分說明：原本這個檔案近2000行所有邏輯都擠在一起，現在拆成：
  ...
  capital.py  — 虛擬(模擬)/真實帳戶可用資金判斷，唯一規則：模擬模式只看paper_capital，
                真實模式才看真實capital，兩者永遠不互相影響。拆出來方便寫單元測試
                (tests/test_capital_separation.py)鎖住這個規則，不會再回歸成「真實帳戶
                0元，連帶卡死模擬模式」這種bug。
...
安全性：所有API路由(除了 / 跟 /health)現在都需要 X-API-Key header(見下方BACKEND_API_KEY)，
CORS允許來源也改成可由環境變數FRONTEND_ORIGIN設定——原本完全公開、任何人都能呼叫
/auto/start對你已經連線的真實帳戶送出真實委託，這是最高優先要補上的洞。
"""
...
from capital import get_paper_available_capital, get_effective_capital
from config import (
    TW_TZ, tw_now, FEE_RATE, FEE_DISCOUNT, DAYTRADE_TAX_RATE, ETF_TAX_RATE, is_etf,
...
def _record_paper_validation(pnl:float=0.0, pct:float=0.0, held_min:float=0.0, sym:str="", tag:str=""):
    ...
    修正③：使用者實際回報過「模擬驗證顯示1/20筆，但今天交易紀錄是空的，不可能是1/20」的疑惑——
    根因是trade_history(畫面上「今日交易紀錄」)會被_reset_daily每天清空，但這裡的累積驗證
    刻意跨天不清空(這是正確、刻意的設計，不然每次重新部署/隔天驗證進度就會歸零)，導致
    「今天看起來0筆」跟「累積1/20筆」同時為真、看似矛盾，其實只是兩個資料的保存期限不一樣。
    新增trade_log(放在paper_validation底下，所以跟trade_count一樣不會被每日重置)，逐筆記錄
    日期/股票代號/損益/結果，讓使用者可以實際點開來看這20筆裡每一筆是哪天的哪檔股票，
    不用再單憑一個數字猜測或懷疑系統算錯——這個函式上線後才開始記錄，這次之前已經算進
    trade_count的舊紀錄(包含使用者問到的這個「1」)沒有對應明細可以回溯，trade_log會顯示
    比trade_count少1筆，這是已知限制，不是新bug。"""
    ...
    if pnl>0:
        ...
        pv["current_streak"]=0
    pv.setdefault("trade_log",[]).append({
        "date":today_str,"time":tw_now().strftime("%H:%M:%S"),
        "sym":sym,"pnl":round(pnl,0),"pct":round(pct,2),"tag":tag,
        "result":"win" if pnl>0 else "loss" if pnl<0 else "breakeven",
    })
    pv["trade_log"]=pv["trade_log"][-50:]  # 上限50筆，遠超過20筆驗證門檻，不會無限長大
...
def auto_trade_tick():
    ...
    # 虛實帳戶分離規則唯一的判斷點是capital.py的get_effective_capital()——模擬模式看paper_capital，
    # 真實模式才看真實capital，兩者互不影響(不會再發生「真實帳戶0元，連帶卡死模擬模式」這種bug，
    # 已寫進tests/test_capital_separation.py鎖住)。
    effective_capital=get_effective_capital(auto_state)
    ...
    for pos in auto_state["positions"]:
      try:
        ...
        min_move=min_profitable_move_pct(auto_state.get("fee_discount",FEE_DISCOUNT),symbol=sym)
        ...
        if close:
            ...
            cost=calc_round_trip_cost(entry_p,exit_p,shares,auto_state.get("fee_discount",FEE_DISCOUNT),symbol=sym)
            ...
            if auto_state.get("paper_mode") and pos["dir"]=="L":
                settlement_date=_record_paper_close(fill_p,shares,tw_now().strftime("%Y-%m-%d"),symbol=sym)
            ...
            if auto_state.get("paper_mode"):
                _record_paper_validation(net_pnl,pp,held_min,sym=sym,tag=tag)
        ...
        try:
            ...
            if auto_state.get("paper_mode"):
                _record_paper_validation(net_pnl,pp_fc,(time.time()-pos.get("opened_at",time.time()))/60,
                                          sym=sym,tag="強制平倉" if is_scheduled else "緊急停止平倉")
...
# get_paper_available_capital()/get_effective_capital() 已搬到capital.py(虛實帳戶分離規則
# 唯一的判斷點，從模組最上面 from capital import ... 取得)，這裡不再重複定義。
...
def _record_paper_close(exit_price:float, shares:int, close_date_str:str, symbol:str=None):
    ...
    修正兩處：①原本sell_fee固定用模組預設的6折(FEE_DISCOUNT)，沒有跟著auto_state["fee_discount"]
    (使用者透過/auto/fee-discount實際設定的折扣)走，導致這裡算出來、實際存進paper_capital的金額，
    跟daily_pnl那邊(呼叫calc_round_trip_cost時有正確帶auto_state["fee_discount"])可能對不起來，
    使用者會看到「今日損益」跟「總資產變化」兩個數字不一致，看起來像帳算錯了。
    ②原本tax固定用個股當沖稅率0.15%，00631L/009816這種ETF實際當沖稅率是0.1%，沒給symbol時
    這裡會多收0.05個百分點，讓paper_capital的實際入帳金額比真實情況更少——傳symbol進來就會自動修正。"""
    fee_discount=auto_state.get("fee_discount",FEE_DISCOUNT)
    sell_fee=max(20,exit_price*shares*FEE_RATE*fee_discount)
    tax_rate=ETF_TAX_RATE if (symbol and is_etf(symbol)) else DAYTRADE_TAX_RATE
    tax=exit_price*shares*tax_rate
...
# ── CORS：可由環境變數FRONTEND_ORIGIN設定成你前端的實際網址(逗號分隔可填多個)，
#    不設定的話退回開放所有來源(方便本機開發)，但會在log留一筆警告，提醒部署到正式環境時應該設定。
#    重要：擋掉外人的關鍵不是CORS(那是瀏覽器才會遵守的規則，curl/Postman完全不受影響)，
#    是下面的BACKEND_API_KEY——CORS只是多一層，沒有BACKEND_API_KEY的話這裡設再嚴都沒用。
_frontend_origin_env = os.environ.get("FRONTEND_ORIGIN", "").strip()
ALLOWED_ORIGINS = [o.strip() for o in _frontend_origin_env.split(",") if o.strip()] or ["*"]
if ALLOWED_ORIGINS == ["*"]:
    logger.warning("⚠️ 尚未設定環境變數FRONTEND_ORIGIN，CORS暫時開放所有來源；正式環境建議設定成你前端的實際網址")
app.add_middleware(CORSMiddleware,allow_origins=ALLOWED_ORIGINS,allow_methods=["*"],allow_headers=["*"])

# ── API Key驗證：真正擋住「外人直接呼叫這支API」的關鍵一層。
#    設定環境變數BACKEND_API_KEY後，除了/跟/health(健康檢查用，Railway等平台會定期打)以外，
#    所有路由都要求Header帶X-API-Key且完全相符，否則401。
#    沒設定BACKEND_API_KEY時整個檢查直接跳過(方便本機開發不用額外設定)，但會在啟動時big警告一次——
#    部署到正式環境(尤其要連真實帳戶/送真實委託)前，務必設定這個環境變數。
BACKEND_API_KEY = os.environ.get("BACKEND_API_KEY", "").strip()
_PUBLIC_PATHS = {"/", "/health"}
if not BACKEND_API_KEY:
    logger.warning("⚠️⚠️⚠️ 尚未設定環境變數BACKEND_API_KEY，後端目前任何人都能直接呼叫所有API(包含/auto/start送出真實委託)！"
                    "正式使用前請務必到Railway設定這個環境變數，並在前端(Vercel)的api/sinopac.js用同一個值打過來。")

@app.middleware("http")
async def _verify_backend_api_key(request: Request, call_next):
    if BACKEND_API_KEY and request.url.path not in _PUBLIC_PATHS:
        provided = request.headers.get("x-api-key", "")
        if provided != BACKEND_API_KEY:
            return JSONResponse(status_code=401, content={"detail": "未授權：缺少或錯誤的 X-API-Key"})
    return await call_next(request)

class ConnectRequest(BaseModel):
    # 修正：api_key/secret_key現在預設留空——優先讀環境變數SINOPAC_API_KEY/SINOPAC_SECRET_KEY，
    # 沒設定環境變數時才退回用請求帶的值。這樣永豐金鑰可以完全不經過瀏覽器/不存在使用者裝置上，
    # 前端也不用每次手動輸入，連線只是單純按一個按鈕。
...
def get_status():
    ...
    return {
        ...
        "daily_pnl_pct":round(auto_state["daily_pnl"]/(get_effective_capital(auto_state) or 1)*100,2),
        "win_rate":wr,
...
async def connect(req:ConnectRequest, background_tasks:BackgroundTasks):
    ...
    # 優先用環境變數(SINOPAC_API_KEY/SINOPAC_SECRET_KEY)，請求沒帶/環境變數沒設定時才看請求body——
    # 這樣金鑰可以完全只活在Railway後端的環境變數裡，不需要在前端輸入、不會存在使用者瀏覽器/裝置上。
    api_key = os.environ.get("SINOPAC_API_KEY","").strip() or req.api_key.strip()
    secret_key = os.environ.get("SINOPAC_SECRET_KEY","").strip() or req.secret_key.strip()
    if not api_key or not secret_key:
        raise HTTPException(status_code=400,detail="缺少永豐 API Key/Secret Key：請在後端(Railway)設定環境變數 "
                                                     "SINOPAC_API_KEY/SINOPAC_SECRET_KEY，或在請求中提供")
    try:
